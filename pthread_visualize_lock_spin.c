...
/* unlockを呼び出す; 前で記録を取る */
int unlock_vis(spinlock_t * l,
	       record_buf * R, long x) {
  enter_unlock(R, x);
  return spin_unlock(l);
}

/* 記録をファイルに保存 */
spinlock_t file_mutex[1];
void dump_record_buf(record_buf * R, int idx, FILE * wp) {
  long n = R->n;
  long i;
  spin_lock(file_mutex);
  for (i = 0; i < n; i++) {
